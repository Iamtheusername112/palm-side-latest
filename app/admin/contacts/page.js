...
const AdminContactsPage = () => {
  ...
  const updateContact = async (contactId, updates) => {
    try {
      ...
      // Get the current contact to track status changes
      ...
      if (result.success) {
        ...
        // Update contact counts in context if status changed
        if (updates.status && oldStatus !== updates.status) {
          updateContactCounts({ from: oldStatus, to: updates.status })
        }

        // Refresh contact counts to ensure accuracy
        fetchContactCounts()
          ...
          <div className='relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white'>
            <div className='mt-3'>
              ...
              <div className='mt-6 flex justify-end space-x-3'>
                ...
                <button
                  onClick={() =>
                    updateContact(selectedContact.id, { status: 'responded' })
                  }
                  disabled={updating}
