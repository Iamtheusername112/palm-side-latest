...
const AdminDashboard = () => {
  ...
  const fetchStats = async () => {
    try {
      ...
      const propertiesData = await propertiesResponse.ok ? await propertiesResponse.json() : { pagination: { total: 0 } }
      
      // Fetch contacts count
      const contactsResponse = await fetch('/api/admin/contacts?limit=1')
      const contactsData = await contactsResponse.ok ? await contactsResponse.json() : { pagination: { total: 0 } }
      
      // Fetch new contacts count
      const newContactsResponse = await fetch('/api/admin/contacts?status=new&limit=1')
      const newContactsData = newContactsResponse.ok ? await newContactsResponse.json() : { pagination: { total: 0 } }

      setStats(prev => prev.map(stat => {
        switch (stat.title) {
          case 'Total Properties':
            return { ...stat, value: propertiesData.pagination?.total?.toString() || '0' }
          case 'Active Listings':
            return { ...stat, value: propertiesData.pagination?.total?.toString() || '0' }
          case 'Total Clients':
            return { ...stat, value: '0' } // Will implement when clients API is ready
          case 'New Contacts':
            return { ...stat, value: newContactsData.pagination?.total?.toString() || '0' }
          case 'Total Revenue':
    ...
    try {
      ...
      setContactStats({
        byStatus: [
          ...
          { status: 'Responded', count: 4, percentage: 20, color: 'bg-green-500' },
          { status: 'Closed', count: 2, percentage: 10, color: 'bg-gray-500' }
        ],
        byPriority: [
          { priority: 'Urgent', count: 3, percentage: 15, color: 'bg-red-500' },
          { priority: 'High', count: 5, percentage: 25, color: 'bg-orange-500' },
          { priority: 'Normal', count: 10, percentage: 50, color: 'bg-blue-500' },
          { priority: 'Low', count: 2, percentage: 10, color: 'bg-gray-500' }
        ],
        bySource: [
          { source: 'Website', count: 15, percentage: 75, color: 'bg-blue-500' },
                    ...
                    {recentActivities.map((activity) => (
                      ...
                      >
                        <div className="flex items-center">
                          <div className={`p-2 rounded-full bg-gray-100 ${activity.color} mr-3`}>
            ...
            <div className="space-y-6">
              <div className="bg-white rounded-xl shadow-sm border p-6">
                ...
                <div className="space-y-3">
                  <button 
                    onClick={() => router.push('/admin/properties')}
                    className="w-full bg-blue-600 hover:bg-blue-700 text-white px-4 py-3 rounded-lg font-medium transition-colors duration-300 flex items-center justify-center"
                  >
                    <Plus className="h-4 w-4 mr-2" />
                    Add New Property
                  </button>
                  ...
                  <button 
                    ...
                    className="w-full bg-gray-600 hover:bg-gray-700 text-white px-4 py-3 rounded-lg font-medium transition-colors duration-300 flex items-center justify-center"
                  >
